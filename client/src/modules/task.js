                    ...
                    <table className='flex flex-col w-full'>
                        <thead className='sticky top-0'>
                            <tr className='bg-[#4785C1] font-Inter text-white text-sm'>
                                <th scope="col" className={`rounded-l-md font-light h-8 ${props.edit?'w-24':'w-10'}`}>
                                </th>
                            ...
                            <tbody>
                                {props.tasks.map((task)=>
                                    ...
                                    <tr onClick={(e)=>{taskFocus===task.id?setTaskFocus(null):setTaskFocus(task.id)} } className={`ml-2 font-Inter h-14 w-fit text-white text-sm ${taskFocus===task.id?'bg-[#6C93B9]/60':'bg-[#6C93B9]/40'}`}>
                                        <td scope="row" className={`rounded-l-md font-light w-24  h-14 ${props.edit?'w-24':'w-10'}`}>
                                        ...
                                        <td scope="row" className='text-center pl-2 py-2 font-light w-24 '>
                                            {task.name}
                                        </td>
                                        ...
                                        <td className='rounded-r-md border-1 border-black font-light py-3 w-40'>
                                            {task.employeelist.map((emp)=>
                                                emp === userId&&
                                                <Stopwatch isOpen={task.paymenttype} task={task} hourlog={props.hourlog} userId={userId}/>
                                            )}
                                            {/* {props.hourlog&&props.hourlog.map((hl)=>
                                                hl.taskname === task.name&&hl.employeeassigned===userId && 
                                                <Stopwatch isOpen={task.paymenttype} id={task.id} hourlog={hl} userId={userId}/>
                                            )} */}
                                        </td>
                                    </tr>
                                )}
